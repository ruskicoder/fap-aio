/**
 * Form Data helpers for MoveOut (userscript version)
 * Uses native fetch (not polyfilled) and DOMParser (not cheerio)
 */

import { nativeFetch } from '../../../nativeFetch';
import { parseHTML, query, getAttr } from '../dom-helpers';
...
export const secondFormGetter = async (secondId: string, id: string) => {
  const page = await (
    await nativeFetch(url + `?id=${secondId}`, {
      method: "GET",
    })
  ).text();
