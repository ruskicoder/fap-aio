/**
 * MoveOut App (userscript version)
 * Based on standalone fap-moveout with DOMParser instead of cheerio,
 * raw localStorage instead of storage facade, nativeFetch instead of polyfilled fetch
 */

import React, { useEffect, useState } from "react";
import { nativeFetch } from '../../nativeFetch';
import { parseHTML, queryAll, getText, getAttr } from './dom-helpers';
...
export default function App() {
  ...
  useEffect(() => {
    const fetchClassListAndSubjects = async () => {
      nativeFetch(`https://fap.fpt.edu.vn/Course/Groups.aspx?group=${id}`)
  ...
  const crawlAndSave = async () => {
    ...
    for (const [key, _item] of classes) {
      ...
      let nextClass;
      nextClass = await (
        await nativeFetch(window.location.href, {
          method: "POST",
          headers: {},
          body: formData,
        })
